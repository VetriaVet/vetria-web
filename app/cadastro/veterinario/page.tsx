...
// Cadastro do veterinário (TASK-005). signUp LIGADO (padrão /login, role=vet no
// metadata → trigger grava o role, migration 0001). Email confirmation ON (DL-009).
// Após confirmar e logar → /app/veterinario/onboarding (multi-step).
//
// Deu certo → o formulário SAI e entra o estado de confirmação (mesma tela, o
// painel da esquerda continua). Não redirecionamos pro /login: a conta nasce sem
// email confirmado e o Supabase recusaria a senha recém-criada.

const FEATURES = [
  "Perfil profissional verificado",
  "Encontrado por responsáveis da sua região",
  "Reputação validada por avaliações reais",
];

// Tempo que o check do botão fica visível antes da troca de estado.
const PAUSA_DO_CHECK = 900;

export default function CadastroVetPage() {
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    const { error } = await supabase.auth.signUp({
      ...
      password: senha,
      options: {
        data: { full_name: nome, cidade, role: "vet" },
      ...
      <div className="flex-1 grid lg:grid-cols-2 max-w-5xl mx-auto w-full">
        ...
        <aside className="hidden lg:flex flex-col justify-center bg-principal text-white rounded-2xl m-6 p-10">
          ...
          <h2 className="font-bold text-2xl leading-tight mb-6">
            Seja encontrado pelos responsáveis certos.
          </h2>
            ...
            {confirmacao ? (
              ...
              <>
                ...
                <p className="text-[15px] text-corpo-texto mb-8">
                  Depois você completa o perfil profissional e validamos seu CRMV.
                </p>

                <form onSubmit={handleSubmit} className="flex flex-col gap-4">
                  <div>
                    <Label htmlFor="nome">Nome completo</Label>
                    <Input id="nome" value={nome} onChange={(e) => setNome(e.target.value)} required placeholder="Como no CRMV" autoComplete="name" />
                  </div>
                  <div>
                    <Label htmlFor="email">Email</Label>
                    <CampoMascarado id="email" mascara="email" valor={email} onValor={setEmail} required placeholder="seu@email.com" />
                  </div>
                  <div>
