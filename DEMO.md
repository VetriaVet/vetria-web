...
## ✅ Antes de começar (checklist que evita tropeço ao vivo)

- [ ] **Acordar o Supabase.** O projeto free pausa por inatividade. Abra o app uns 10 min antes; se o login der "Failed to fetch", entre no dashboard do Supabase e restaure o projeto.
- [ ] **Conta de demo pronta e confirmada.** O envio de email (Resend) está em **modo teste**: a confirmação só chega no endereço verificado. Tenha 1 conta de cada papel **já criada e confirmada**:
  - 1 tutor · 1 veterinário · 1 clínica · 1 admin
- [ ] Abra numa **janela anônima/deslogada** pra começar pela Home pública.
- [ ] Tenha os 4 logins à mão pra alternar entre os papéis sem perder o ritmo.
...
- O CTA **"Você é veterinário ou clínica?"** que leva ao cadastro profissional

> Fala-chave: *"Tudo no design final. A busca e os perfis ligam na próxima fase (backend)."*

### 2. Cadastro + login (REAL)
- Clicar **Criar conta** / **Sou veterinário** pra mostrar o funil separado: tutor (rápido) vs vet/clínica (B2B)
- Login com email/senha **funciona de verdade** (e Google também)
...
- Dashboard: **atividade** e **stats** em esqueleto (ghost), que mostra o formato sem inventar número
...
- Mesma base do vet, mais **Equipe** (lista de profissionais em esqueleto + fluxo de convite)
...
- Separação tutor (B2C) vs vet/clínica (B2B)
...
- **Empty calmo** nos estados positivos sem pendência (próximas consultas, pets, agenda)
- Estados **honestos**: nenhum número, perfil ou preço inventado
